...
class Job(threading.Thread):
    """Job class for running a callable in a separate thread."""
...
def service_shutdown(signum, frame):
    """Handler function to be called when a signal is received"""
    print("exiting ...")
    raise ServiceExit


class tpqoa(DataProvider):
    ...
    def __init__(self, conf_file):
        ...
        self.ctx = v20.Context(
            hostname=self.hostname, port=443, token=self.access_token, poll_timeout=10
        )
    ...
    def get_instruments(self):
        ...
        instruments = [(ins["displayName"], ins["name"]) for ins in instruments]
    ...
    def get_history(self, instrument, start, end, granularity, price, localize=True):
        ...
        if (
            ...
            for t in range(len(dr)):
                ...
                batch = self.retrieve_data(
                    instrument, batch_start, batch_end, granularity, price
                )
                data = pd.concat([data, batch])
        else:
            start = self.transform_datetime(start)
            end = self.transform_datetime(end)
            data = self.retrieve_data(instrument, start, end, granularity, price)
        if localize:
    ...
    def stream_data_failsafe(self, instrument, callback=None):
        ...
        try:
            price_stream_thread = Job(
                self._stream_data_failsafe_thread, [instrument, callback]
            )
